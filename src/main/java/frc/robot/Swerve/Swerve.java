...
public class Swerve extends SubsystemBase {
	...
	public void drive(ChassisSpeeds chassisSpeeds) {
		SwerveModuleState[] swerveModuleStates = SwerveConstants.kSwerveKinematics.toSwerveModuleStates(chassisSpeeds);
		...
		if (Math.abs(angleDiff.getDegrees()) < SwerveConstants.kDriveAssistThreshold) {
			...
			ChassisSpeeds driveAssist = new ChassisSpeeds(
					...
					_driveAssistYPID.calculate(RobotState.getRobotPose().getY(), targetPose.getY())
							* SwerveConstants.maxSpeed,
					_anglePID.calculate(anglePIDMeasurement, anglePIDSetpoint) * SwerveConstants.maxAngularVelocity);
	...
	//     * Calculates the drive assist
	//     *
	//     * @param movingDirection - the direction the robot is moving according to the driver input,
	// robot
	//     *     relative
	//     * @param rotation - the rotation movement of the robot according to the driver input, robot
	// relative
	//     * @param noteTranslation - the translation between the robot and the note, robot relative
	//     * @return Calculated drive assist as chassis speeds(if the rotation is 0, use the rotation
	// from
	//     *     the drive input), robot relative
	//     */
	//    private ChassisSpeeds calculateNoteDriveAssist(
	//      Translation2d movingDirection, double rotation, Translation2d noteTranslation) {
	//        if (movingDirection.getX() == 0 && movingDirection.getY() == 0)
	//            return new ChassisSpeeds(0, 0, rotation);
	//
	//        Translation2d toTargetDirection = noteTranslation;
	//
	//        Rotation2d movingAngle = movingDirection.getAngle();
	//        Rotation2d toTargetAngle = toTargetDirection.getAngle();
	//        Rotation2d angleDiff = toTargetAngle.minus(movingAngle);
	...
	//        if (Math.abs(angleDiff.getDegrees()) < SwerveConstants.kDriveAssistThreshold) {
	//            double anglePIDMeasurement = RobotState.getRobotPose().getRotation().getDegrees();
	//            double anglePIDSetpoint = toTargetAngle.getDegrees();
	//
