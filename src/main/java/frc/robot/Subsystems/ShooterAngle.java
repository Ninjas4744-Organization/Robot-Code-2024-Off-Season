        ...
        addFunctionToPeriodicMap(
            () -> {
                double dist = RobotState.getRobotPose()
                    .getTranslation()
                    .getDistance(
                        VisionConstants.getTagPose(15).getTranslation());

                Rotation2d angle = Rotation2d.fromRadians(Math.atan2(ShooterAngleConstants.kTargetHeight - ShooterAngleConstants.kShooterHeight, dist));
                double angleClamped = Math.min(Math.max(angle.getDegrees() - ShooterAngleConstants.kShooterStartAngle, 0), 120);
